...
// PENINGKATAN: Menambahkan handler untuk riwayat dan reset formula.

const log = require('electron-log');
const defaultFormulas = require('../defaultFormulas'); // Impor data default
...
function registerFormulaHandlers(ipcMain, db) {
    ...
    ipcMain.handle('formulas:get-history', async (event, formulaId) => {
        return new Promise((resolve, reject) => {
            const sql = `
                SELECT fh.*, u.full_name as user_name 
    ...
    ipcMain.handle('formulas:reset-default', async (event, formulaKey) => {
        return new Promise((resolve, reject) => {
            ...
            db.run("UPDATE calculation_formulas SET formula_value = ? WHERE formula_key = ?", 
                ...
                function(err) {
                    if (err) {
                        reject(new Error('Gagal mengembalikan formula ke default.'));
                        return;
                    }
                    log.info(`Formula with key ${formulaKey} was reset to default.`);
                    resolve({ success: true });
                }
            );
        });
