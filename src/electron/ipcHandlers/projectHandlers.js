...
// serta penambahan handler baru 'db:get-all-trials' untuk dasbor.
...
function registerProjectHandlers(ipcMain, db) {
        ...
        return new Promise((resolve, reject) => {
            db.serialize(() => {
                ...
                db.get("SELECT * FROM projects WHERE id = ?", [projectId], (err, project) => {
                    ...
                    db.run("INSERT INTO projects (projectName, clientName, createdAt, status) VALUES (?, ?, ?, ?)",
                        [newProjectName, project.clientName, newCreatedAt, project.status],
                        function(err) {
                            if (err) {
                                db.run("ROLLBACK");
                                return reject(new Error(`Gagal membuat duplikat proyek: ${err.message}`));
                            }
                            ...
                            db.all("SELECT * FROM project_trials WHERE project_id = ?", [projectId], (err, trials) => {
                                if (err) {
                                    db.run("ROLLBACK");
                                    return reject(new Error(`Gagal mengambil trial asli: ${err.message}`));
                                }

                                const trialPromises = trials.map(trial => {
    ...
    // BARU: Handler untuk mengambil semua trial dari semua proyek untuk dasbor
