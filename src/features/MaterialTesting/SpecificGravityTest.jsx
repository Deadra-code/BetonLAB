...
import { Loader2, Scale } from 'lucide-react'; // PENINGKATAN: Import ikon
...
const TestForm = ({ onSave }) => {
    ...
    const calculation = useMemo(() => {
        ...
        if (typeof dry_weight !== 'number' || typeof ssd_weight !== 'number' || typeof in_water_weight !== 'number') return {};
        if ((ssd_weight - in_water_weight) === 0 || dry_weight === 0 || (dry_weight - in_water_weight) === 0) return {};
    ...
    const canSave = useMemo(() => {
        return inputData.dry_weight > 0 &&
               inputData.ssd_weight > 0 &&
               inputData.in_water_weight > 0 &&
               inputData.ssd_weight > inputData.dry_weight &&
               inputData.ssd_weight > inputData.in_water_weight;
    }, [inputData]);
    ...
    return (
        <Dialog open={isOpen} onOpenChange={setIsOpen}>
            ...
            <DialogContent className="max-w-2xl">
                <DialogHeader>
                    <DialogTitle>Pengujian Berat Jenis & Penyerapan Baru</DialogTitle>
                    <DialogDescription>
                        Masukkan berat benda uji dalam kondisi kering oven, SSD, dan di dalam air.
                    </DialogDescription>
                </DialogHeader>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6 py-4">
                    ...
                    <div className="space-y-2">
                         ...
                         <p>BJ Kering (A / (B-C)): <span className="font-bold">{calculation.bj_bulk_dry?.toFixed(3) || '-'}</span></p>
                         <p>BJ SSD (B / (B-C)): <span className="font-bold">{calculation.bj_ssd?.toFixed(3) || '-'}</span></p>
                         <p>BJ Semu (A / (A-C)): <span className="font-bold">{calculation.bj_apparent?.toFixed(3) || '-'}</span></p>
                         <p>Penyerapan (((B-A)/A)*100): <span className="font-bold">{calculation.absorption?.toFixed(2) || '-'} %</span></p>
                    </div>
                </div>
